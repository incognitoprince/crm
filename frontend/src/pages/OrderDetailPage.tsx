...
export function OrderDetailPage() {
  ...
  async function change(status: OrderStatus) {
    if (!order) return;
    try { setOrder((await updateOrderStatus(order.id, status)).data); } catch (e) { setError(e instanceof Error ? e.message : "Unable to update order"); }
  }
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    ...
    <div className="grid gap-5 md:grid-cols-3">
      ...
      <section className="rounded-xl border border-sand-100 bg-white p-5 shadow-sm"><p className="text-xs text-slate-500">Shop</p><p className="mt-1 font-semibold">{order.shop?.name ?? "—"}</p><p className="text-sm text-slate-600">{order.shop?.area ?? ""}</p></section>
    </div>
    ...
    <section className="rounded-xl border border-sand-100 bg-white p-5 shadow-sm"><h4 className="font-semibold text-navy-900">Delivery & notes</h4><p className="mt-3 text-sm text-slate-600">Delivery: {order.deliveryDate ? new Date(order.deliveryDate).toLocaleDateString() : "Not scheduled"}</p><p className="mt-2 text-sm text-slate-600">{order.notes || "No notes added."}</p></section>
  </div>;
}
