...
export function OrderDetailPage() {
  ...
  useEffect(() => { if (id) getOrder(id).then(r => setOrder(r.data)).catch(e => setError(e.message)); }, [id]);
  async function change(status: OrderStatus) { if (!order) return; try { setOrder((await updateOrderStatus(order.id, status)).data); } catch (e) { setError(e instanceof Error ? e.message : "Unable to update order"); } }
  ...
  return <div className="mx-auto max-w-5xl space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <section className="rounded-xl border border-sand-100 bg-white p-5 shadow-sm lg:col-span-2">
        <h4 className="font-semibold text-navy-900">Customer reference</h4>
        {order.referenceImages.length ? <div className="mt-4 grid gap-4 sm:grid-cols-2">{order.referenceImages.map(image => <figure key={image.id} className="overflow-hidden rounded-lg border border-slate-200"><img src={image.storagePath} alt={image.fileName} className="max-h-96 w-full object-contain bg-slate-50" /><figcaption className="truncate p-2 text-xs text-slate-500">{image.fileName}</figcaption></figure>)}</div> : <p className="mt-3 text-sm text-slate-500">No reference image attached.</p>}
      </section>
    ...
    <section className="rounded-xl border border-sand-100 bg-white p-5 shadow-sm">
      <h4 className="font-semibold text-navy-900">Production quantity</h4>
