  ...
  return <div className="mx-auto max-w-3xl space-y-4">
    ...
    <article className="rounded-xl border border-slate-200 bg-white p-7 shadow-sm print:border-0 print:p-0 print:shadow-none">
      <header className="flex flex-col gap-5 border-b border-slate-200 pb-6 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">StitchPro</p>
          <h1 className="mt-1 text-2xl font-semibold text-navy-900">Invoice</h1>
          <p className="mt-1 text-sm text-slate-500">{invoice.invoiceNo}</p>
        </div>
        <div className="text-left sm:text-right text-sm">
          <p className="text-slate-500">Issued</p>
          <p className="font-medium text-slate-800">{new Date(invoice.issuedAt).toLocaleDateString()}</p>
        </div>
      </header>
      ...
      <section className="py-6">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-slate-200 text-left text-xs text-slate-500"><th className="pb-3">Description</th><th className="pb-3 text-center">Qty</th><th className="pb-3 text-right">Amount</th></tr></thead>
