import type { Customer, CustomerDetail, DashboardSummary, HealthResponse, Measurement, Order, Shop, GarmentType, Master, Design, ProductionOrder, OrderDesign, MasterAssignment } from "../types";
...
export function linkDesignToOrder(orderId: string, designId: string, notes?: string) { return request<{ data: OrderDesign }>("/api/production/orders/" + orderId + "/designs", { method: "POST", body: JSON.stringify({ designId, notes }) }); }
