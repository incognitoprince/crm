...
// This is the include shape for one OrderDesign record.
// It is intentionally separate from the Order.designs relation args below.
...
router.post("/order-designs/:orderDesignId/assignments", asyncHandler(async (req, res) => {
  ...
  if (!od.order.shopId) {
    throw new AppError(
      "Assign a stitching shop to the order before assigning a master",
      400,
      "ORDER_SHOP_REQUIRED"
    );
  }
  ...
  if (!master || !master.active) {
    throw new AppError("Master not found or inactive", 404, "MASTER_NOT_FOUND");
  }
  ...
  const breakdown = od.order.sizeBreakdowns.find(
    item => item.size.toUpperCase() === input.size.toUpperCase()
  );
  if (!breakdown) {
    throw new AppError("Selected size is not part of this order", 400, "SIZE_NOT_IN_ORDER");
  }
...
router.patch("/assignments/:id", asyncHandler(async (req, res) => {
  ...
  if (
    current.startedAt &&
    input.masterId &&
    input.masterId !== current.masterId
  ) {
    throw new AppError(
      "Master cannot be changed after work has started",
      400,
      "ASSIGNMENT_LOCKED"
    );
  }
  ...
  if (input.masterId) {
    ...
    if (master.shopId !== current.orderDesign.order.shopId) {
      throw new AppError(
        "Master must belong to the order's stitching shop",
        400,
        "MASTER_SHOP_MISMATCH"
      );
    }
  }
  ...
  if (input.quantity !== undefined) {
    const total =
      current.orderDesign.order.sizeBreakdowns.find(s => s.size === current.size)?.quantity ?? 0;

    const other = await prisma.masterAssignment.aggregate({
      where: {
        orderDesignId: current.orderDesignId,
        size: current.size,
        id: { not: current.id },
      },
      _sum: { quantity: true },
    ...
    if ((other._sum.quantity ?? 0) + input.quantity > total) {
      throw new AppError(
        "Assignment exceeds size quantity",
        400,
        "ASSIGNMENT_EXCEEDS_QUANTITY"
      );
    }
...
router.delete("/assignments/:id", asyncHandler(async (req, res) => {
  ...
  if (current.startedAt) {
    throw new AppError("Started assignments cannot be removed", 400, "ASSIGNMENT_LOCKED");
  }
