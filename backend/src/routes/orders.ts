...
router.get("/", asyncHandler(async (req, res) => {
  ...
  const orders = await prisma.order.findMany({ where: status ? { status } : undefined, include: includes, orderBy: { orderDate: "desc" }, take: 100 });
...
router.post("/", asyncHandler(async (req, res) => {
  ...
  if (input.sizeBreakdowns.length && breakdownTotal !== input.quantity) throw new AppError("Size quantities must equal total quantity", 400, "INVALID_SIZE_BREAKDOWN");

  const order = await prisma.order.create({
    data: {
      customerId: input.customerId, shopId: input.shopId ?? null, garment: input.garment, description: input.description,
      quantity: input.quantity, totalAmountFils: input.totalAmountFils, paidAmountFils: input.paidAmountFils, paymentStatus,
      deliveryDate: input.deliveryDate ? new Date(input.deliveryDate) : null, notes: input.notes ?? null,
...
router.post("/:id/reference-images", upload.single("image"), asyncHandler(async (req, res) => {
  ...
  const image = await prisma.orderReferenceImage.create({ data: { orderId: order.id, fileName: req.file.originalname, storagePath: "/uploads/" + finalName, mimeType: req.file.mimetype } });
  res.status(201).json({ data: image });
}));

router.patch("/:id/status", asyncHandler(async (req, res) => {
  const input = z.object({ status: z.enum(statuses) }).parse(req.body);
  const order = await prisma.order.update({ where: { id: req.params.id }, data: { status: input.status }, include: includes }).catch(() => null);
  if (!order) throw new AppError("Order not found", 404, "ORDER_NOT_FOUND");
  res.json({ data: order });
}));
